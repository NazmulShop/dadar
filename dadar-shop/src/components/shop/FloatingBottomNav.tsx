...
export function FloatingBottomNav() {
  ...
  return (
    <nav
      aria-label="Primary"
      className="animate-float-in pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
    >
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.to}
              className={cn(
                "tap-bounce relative flex h-10 items-center justify-center gap-1.5 rounded-pill px-2.5 text-sm font-medium",
                isActive
