...
export const Route = createFileRoute("/admin/orders/delivered")({ component: DeliveredOrdersPage });
...
function DeliveredOrdersPage() {
  ...
  useEffect(() => {
    adminFetch<any[]>("orders")
      .then(d => { setOrders(Array.isArray(d) ? d.filter((o: any) => o.status === "Delivered") : []); setLoading(false); })
  ...
  return (
    <AdminLayout>
      <header className="surface-card mb-4 rounded-3xl p-6">
        <h1 className="text-display flex items-center gap-2 text-3xl font-semibold"><PackageCheck className="size-7 text-emerald-600" /> Delivered Orders</h1>
        <p className="text-muted-foreground mt-1 text-sm">{loading ? "Loading…" : `${filtered.length} successfully delivered`}</p>
      </header>
      ...
      <div className="surface-card rounded-3xl overflow-hidden">
        {loading ? <div className="py-12 text-center text-muted-foreground text-sm">Loading…</div> :
          filtered.length === 0 ? <div className="py-12 text-center text-muted-foreground text-sm">No delivered orders.</div> : (
