...
function ProductFormModal({
  ...
  async function handleImageFile(file: File) {
    ...
    try {
      ...
      const res = await fetch(`${API_ORIGIN}/api/uploads/image`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ source: dataUrl, folder: "dadar-shop/products" }),
      });
      ...
      >
        ...
        <div className="grid gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <label className="bg-surface-muted hover:bg-primary-soft flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-border p-4 text-center transition">
              {imagePreview ? (
              ...
              <span className="text-xs text-muted-foreground">
                {uploading ? "Uploading…" : "Click to upload image (PNG / JPG / WEBP)"}
              </span>
              <input
                type="file"
                accept="image/*"
      ...
      <section className="surface-card overflow-x-auto rounded-3xl p-2">
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody>
            {products.length === 0 && (
              <tr>
                <td colSpan={6} className="text-muted-foreground py-8 text-center text-sm">No products available. Add products first.</td>
              </tr>
            )}
            {products.slice(0, 8).flatMap((p, idx) =>
              SIZES.slice(0, 2).map((s, i) => (
    ...
    <div className="space-y-4">
      <section className="surface-card rounded-3xl p-6">
        ...
        <p className="text-muted-foreground mt-1 text-xs">
          Upload a CSV or XLSX with columns: <code>name, category, brand, price, stock, description</code>.
        </p>
      ...
      <section className="surface-card rounded-3xl p-5">
        ...
        <p className="text-muted-foreground py-4 text-center text-sm">No import history yet. Imports will appear here once you upload a file.</p>
      </section>
    </div>
  );
}
