...
export const Route = createFileRoute("/admin/orders/cancelled")({ component: CancelledOrdersPage });
...
function CancelledOrdersPage() {
  ...
  useEffect(() => {
    adminFetch<any[]>("orders")
      .then(d => { setOrders(Array.isArray(d) ? d.filter((o: any) => ["Cancelled", "Returned"].includes(o.status)) : []); setLoading(false); })
  ...
  return (
    <AdminLayout>
      <header className="surface-card mb-4 rounded-3xl p-6">
        <h1 className="text-display flex items-center gap-2 text-3xl font-semibold"><PackageX className="size-7 text-rose-600" /> Cancelled Orders</h1>
        <p className="text-muted-foreground mt-1 text-sm">{loading ? "Loading…" : `${filtered.length} cancelled or returned`}</p>
      </header>
      ...
      <div className="surface-card rounded-3xl overflow-hidden">
        {loading ? <div className="py-12 text-center text-muted-foreground text-sm">Loading…</div> :
          filtered.length === 0 ? <div className="py-12 text-center text-muted-foreground text-sm">No cancelled orders.</div> : (
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map(o => (
                    <tr key={o.id} className="hover:bg-surface-muted/40">
                      ...
                      <td className="px-4 py-3"><span className="rounded-full bg-rose-100 text-rose-800 px-2 py-0.5 text-[11px] font-semibold">{o.status}</span></td>
