...
/* =========================================================
 * Dadar Shop — Real Auth (Resend OTP + JWT + PostgreSQL)
 * ---------------------------------------------------------
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login: AuthApi["login"] = async (email, password, remember) => {
    const data = await apiCall<{ token: string; expiresAt: number; user: AuthUser }>("/login", {
      method: "POST",
  ...
  const requestOtp: AuthApi["requestOtp"] = async (channel, target) => {
    ...
    // In dev mode (no Resend key) backend logs & returns OTP for easy testing
    return data.devOtp ?? "";
  };

  const verifyOtp: AuthApi["verifyOtp"] = async (target, code) => {
    const data = await apiCall<{ token: string; expiresAt: number; user: AuthUser }>("/verify-otp", {
      method: "POST",
  ...
  const verifyEmail: AuthApi["verifyEmail"] = async (code) => {
    const token = getToken();
    const data = await apiCall<{ user: AuthUser }>("/verify-email", {
      method: "POST",
