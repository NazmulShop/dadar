...
// ───── Shipping Zones ─────
export const shippingZonesTable = sqliteTable("shipping_zones", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
  areas: text("areas").notNull().default(""),
  charge: integer("charge").notNull().default(0),
  estimatedDays: text("estimated_days").notNull().default(""),
  active: integer("active", { mode: "boolean" }).notNull().default(true),
  sortOrder: integer("sort_order").notNull().default(0),
  createdAt: ts("created_at", { notNull: true, default: true }),
  updatedAt: ts("updated_at", { notNull: true, default: true }),
...
// ───── Message Template Overrides (admin-edited email/SMS copy) ─────
// Defaults for every event live client-side in NOTIFICATION_TEMPLATES
// (src/data/account.ts). When an admin edits a template here, the override
// row wins; unedited events keep using the shipped default text.
...
// ───── System (Brevo) Email Templates ─────
// These are the actual transactional emails Brevo sends (registration OTP,
// admin login OTP, email-verify/login OTP, password reset). Unlike
// messageTemplatesTable above (cosmetic per-event copy shown in the admin
// notification-template pages), an override row here is read by
// lib/email.ts at send time and changes what customers actually receive.
export const authEmailTemplatesTable = sqliteTable("auth_email_templates", {
  event: text("event", {
    enum: ["register_otp", "admin_login_otp", "email_verify_otp", "otp_login", "password_reset"],
  }).primaryKey(),
  subject: text("subject"),
