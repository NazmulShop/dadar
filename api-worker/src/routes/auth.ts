...
// ─── D1-backed bootstrap rate limit (per IP, 5 / hour) ─────
// Was KV-backed (`RATE_KV`). Moved to D1: the Workers KV free plan caps
// writes at 1,000/day account-wide, and that quota was being exhausted by
// the admin login flow's own KV writes (see lib/rateLimit.ts and the ticket
// helpers below), which then made *this* function's unguarded
// `RATE_KV.put()` throw "KV put() limit exceeded for the day" too.
async function bootstrapRateLimit(env: Env, ip: string): Promise<boolean> {
  return rateLimit(env, `bootstrap:${ip}`, 5, 60 * 60 * 1000);
}

// ─── D1-backed Super Admin login ticket (password -> OTP -> secret) ────────
// Gatekeeps the 3-step admin login so each step can only be reached after
// the previous one succeeded. Stored in D1 (`admin_login_tickets` table),
...
//
// Was KV-backed. Moved to D1 because Workers KV's free-plan write quota
// (1,000 `put`s/day, account-wide) was being exhausted by normal admin
// login traffic — every login attempt wrote to KV 3-4 times (issue ticket
// here, rate-limit counters in lib/rateLimit.ts, ticket advance here
// again), and once exhausted every further `RATE_KV.put()` threw "KV put()
// limit exceeded for the day", which is exactly the error blocking admin
// login. D1's free-plan write allowance is far larger.
...
async function issueAdminLoginTicket(
  ...
  await getDb(env).insert(adminLoginTicketsTable).values({
    id: ticketId,
    userId: data.userId,
    email: data.email,
    remember: data.remember,
    stage: "password_ok",
    expiresAt: new Date(Date.now() + ADMIN_LOGIN_TICKET_TTL_MS),
  });
  return ticketId;
}
...
async function readAdminLoginTicket(env: Env, ticketId: string): Promise<AdminLoginTicket | null> {
  ...
  const rows = await getDb(env)
    .select()
    .from(adminLoginTicketsTable)
    .where(eq(adminLoginTicketsTable.id, ticketId))
    .limit(1);
  const row = rows[0];
  if (!row) return null;
  if (!row.expiresAt || row.expiresAt.getTime() < Date.now()) return null;
  return { userId: row.userId, email: row.email, remember: row.remember, stage: row.stage };
}

async function advanceAdminLoginTicket(env: Env, ticketId: string, ticket: AdminLoginTicket): Promise<void> {
  // Must be an upsert, not update-only: the old KV `.put()` this replaced
  // created-or-overwrote the record. One caller (the passwordless
  // /verify-otp route, for an already-verified Super Admin email) hands in
  // a brand-new ticketId that has never been inserted yet — an
  // UPDATE-only version silently matches zero rows there, so the ticket
  // never actually gets created and the next step ("Verification session
  // expired") fails even seconds later. This was the bug behind that.
  const expiresAt = new Date(Date.now() + ADMIN_LOGIN_TICKET_TTL_MS);
  await getDb(env)
    .insert(adminLoginTicketsTable)
    .values({
      id: ticketId,
    ...
    .onConflictDoUpdate({
      target: adminLoginTicketsTable.id,
      set: {
        userId: ticket.userId,
        email: ticket.email,
        remember: ticket.remember,
...
async function consumeAdminLoginTicket(env: Env, ticketId: string): Promise<void> {
  await getDb(env).delete(adminLoginTicketsTable).where(eq(adminLoginTicketsTable.id, ticketId));
}
