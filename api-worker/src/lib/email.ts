...
function buildResetHtml(opts: {
  ...
  <div class="wrap">
    ...
    <p class="sub">Hi ${escHtml(opts.name)}, we received a request to reset your password.</p>
...
export async function sendRegistrationOtp(
  ...
  const html = buildOtpHtml({
    greeting: `Hi ${escHtml(opts.name)}, verify your email`,
...
export async function sendAdminLoginOtp(
  ...
  const html = buildOtpHtml({
    ...
    bodyText: `Hi ${escHtml(opts.name)}, enter this code to continue signing in:`,
    otp: opts.otp,
    expiryNote: "⏱ This code expires in <strong>60 seconds</strong>.",
    footerNote:
      "If you didn't attempt to sign in, secure your account immediately.",
  });
...
export async function sendGeneralOtp(
  ...
  const html = buildOtpHtml({
    ...
    bodyText: opts.name ? `Hi ${escHtml(opts.name)}, ${config.body.toLowerCase()}` : config.body,
    otp: opts.otp,
    expiryNote: "⏱ This code expires in <strong>60 seconds</strong>.",
